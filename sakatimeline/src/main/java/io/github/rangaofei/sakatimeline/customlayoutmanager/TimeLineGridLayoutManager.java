...
public class TimeLineGridLayoutManager extends PerfectLinearLayoutManager {
     ...
     */
    public TimeLineGridLayoutManager(Context context, AttributeSet attrs, int defStyleAttr,
                                     int defStyleRes) {
     ...
     */
    public TimeLineGridLayoutManager(Context context, int spanCount) {
        super(context);
...
//                final Rect decorInsets = lp.mDecorInsets;
                int top = getTopDecorationHeight(view);
                int bottom = getBottomDecorationHeight(view);
                int left = getLeftDecorationWidth(view);
                int right = getRightDecorationWidth(view);
                final int verticalInsets = top + bottom
                        + lp.topMargin + lp.bottomMargin;
                final int horizontalInsets = left + right
...
//                    }
                    ...
                    if (i % 2 == 1) {
                        ...
                        right = getPaddingLeft() + mCachedBorders[params.mSpanIndex+1];
                        left = right - mOrientationHelper.getDecoratedMeasurementInOther(view);
                    }
                }
            } else {
...
//        final Rect decorInsets = lp.mDecorInsets;
        ...
        int left = getLeftDecorationWidth(view);
        int right = getRightDecorationWidth(view);
        final int verticalInsets = top + bottom + lp.topMargin + lp.bottomMargin;
        final int horizontalInsets = left + right + lp.leftMargin + lp.rightMargin;
